...
for PRODUCT in "${PRODUCTS[@]}"; do
    ...
    psql -d mm_database -U root -a -v "ON_ERROR_STOP=1" -v file="${FILE_PREFIX}-Blocks-IPv4.csv" -f /share/sql/copy_network.sql
    psql -d mm_database -U root -a -v "ON_ERROR_STOP=1" -v file="${FILE_PREFIX}-Blocks-IPv6.csv" -f /share/sql/copy_network.sql
